...
void _slow_hash(const string &msg, const SecByteBlock& salt, SecByteBlock &hash, bool MHF_ON) {
...
bool harden_pw(const string pw, SecByteBlock& salt, SecByteBlock& key, bool MHF_ON) {
    ...
    if (key.empty()) {
        _slow_hash(pw, salt, key, MHF_ON);
        return false;
    } else {
        SecByteBlock n_key;
        _slow_hash(pw, salt, n_key, MHF_ON);
        return (n_key == key);
    }
}
...
bool pwdecrypt(const string &pw, const string &ctx, string &msg, bool MHF_ON) {
    bool ret = false;
    SecByteBlock key;
    try {
        msg.clear();
