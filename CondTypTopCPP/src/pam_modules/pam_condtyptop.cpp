...
#define TYPTOP_BIN_LOC "/usr/local/bin/condtyptop"
#endif
...
call_typtop(pam_handle_t *pamh, const char *user, const char *passwd, int chkwd_ret) {
    ...
    string cmd = "/usr/local/bin/condtyptop --check " + string(user);
    ...
    if (fp == NULL) {
        pam_syslog(pamh, LOG_ERR, "CondTyptop could not be opened. Sorry! retval=%d\n", retval);
