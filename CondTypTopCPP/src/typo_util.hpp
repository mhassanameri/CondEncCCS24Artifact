...
// Modified By Mohammad Hassan Ameri Sept./08/2024
//

#ifndef CONDTYPTOP_C_TYPO_UTIL_HPP
#define CONDTYPTOP_C_TYPO_UTIL_HPP
...
/*make sure to rename the function for a proper name */
...
inline string get_install_id() {
    ...
    if (f.good() && (f >> id) && id.size()>=8) {
        ...
        id = CryptoSymWrapperFunctions::Wrapper_b64encode(b);
 ...
 */
inline bool meets_typo_policy(const string& pw, const string& typo, const condtyptop::TypoPolicy& tp) {
