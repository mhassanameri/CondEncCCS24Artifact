...
void CryptoSymWrapperFunctions::Wrapper_b64encode(const CryptoPP::byte* raw_bytes, ulong len, string& str) {
...
string CryptoSymWrapperFunctions::Wrapper_b64encode(const string& in) { string out; Wrapper_b64encode((const CryptoPP::byte*)in.data(), in.size(), out); return out; }
